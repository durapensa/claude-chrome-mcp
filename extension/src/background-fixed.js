// Chrome Extension Background Service Worker - FIXED VERSION
// Fixes Chrome freezing issue by reducing aggressive timers and connection attempts
...
// Prevent multiple alarm creation
let alarmCreated = false;

// Keep service worker alive with LESS aggressive timing
chrome.runtime.onStartup.addListener(() => {
  console.log('CCM Extension: Service worker started');
  if (!alarmCreated) {
    chrome.alarms.create('keepAlive', { periodInMinutes: 1 }); // Changed from 0.25 to 1 minute
    alarmCreated = true;
  }
});
...
// Initialize components with delay to prevent immediate connection attempts
let hubClient = null;
let contentScriptManager = null;

// Delay initialization to prevent Chrome freeze
setTimeout(() => {
  console.log('CCM Extension: Delayed initialization starting...');
...
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === 'keepAlive') {
    ...
    // Only check connection if hubClient is initialized
    if (hubClient) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (!hubClient || !contentScriptManager) {
    console.log('CCM Extension: Components not yet initialized');
    sendResponse({ success: false, error: 'Extension still initializing' });
    return false;
  }
...
console.log('CCM Extension: Modular background service worker initialized with fixes');